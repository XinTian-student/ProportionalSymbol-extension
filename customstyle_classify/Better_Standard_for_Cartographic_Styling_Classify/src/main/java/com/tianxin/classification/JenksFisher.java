...
/**
 *
 * Basic port of original C code from Maarten Hilferink. All credits this
 * fantastic work go to him for.
 *
 *
 *
...
public class JenksFisher {
	...
	/**
	 * Constructor that initializes main variables used in fisher calculation of
	 * natural breaks.
	 *
	...
	public JenksFisher(List<ValueCountPair> vcpc, int k) {
		...
		for (int i = 0; i != this.numValues; ++i) {
			currPair = vcpc.get(i);
			assert (i == 0 || currPair.getValue() >= vcpc.get(i - 1).getValue()); // PRECONDITION: the value sequence
			...
			if (i < this.bufferSize) {
				this.previousSSM[i] = cwv * cwv / cw; // prepare sum of squared means for first class. Last (k-1) values
														// are omitted
			}
		}
	...
	private int getSumOfWeights(int b, int e) {
		assert (b != 0); // First element always belongs to class 0, thus queries should never include
							// it.
		assert (b <= e);
	 ...
	 * bp+(completedRows-1) and less than ep+(completedRows-1) Complexity: O(ep-bp)
	 * <= O(m) @
	 *
	 * @param i  startIndex
	 * @param bp endindex
	...
	/**
	 * Find CB[i+completedRows] for all i>=bi and i<ei given that the results are at
	 * least bp+(completedRows-1) and less than ep+(completedRows-1) Complexity:
	 * O(log(ei-bi)*Max((ei-bi),(ep-bp))) <= O(m*log(m))
	 *
	 *
	 * @param bi
	 * @param ei
	...
	private static List<ValueCountPair> getValueCountPairs(List<Double> values) {
		...
		for (Double value : values) {
			if (value != null && !vcpMap.containsKey(value)) {
